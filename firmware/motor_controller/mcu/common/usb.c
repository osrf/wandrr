...
void usb_init()
{
  ...
  g_usb_device_descriptor[10] = 0x23; // pid
...
bool usb_tx(const uint8_t ep, const uint8_t *payload, const uint8_t payload_len)
{
  ...
  USB_INEP(ep)->DIEPTSIZ = (1 << 19) | payload_len; // set outbound pkt size
...
bool usb_setup_rx(uint8_t *buf, const uint32_t len)
{
  ...
  else if (request_type == 0 && request == 0x9) // set configuration
  {
    ...
    USB_INEP(1)->DIEPCTL = USB_OTG_DIEPCTL_SNAK    | // set NAK bit
                           ...
                           USB_OTG_DIEPCTL_USBAEP  | // active endpoint (always 1)
    ...
    g_usbd->DAINTMSK |= 0x40002; // OUT2 and IN1 IRQ
...
void otg_fs_vector()
{
  if (USB_OTG_FS->GINTSTS & USB_OTG_GINTSTS_USBRST)
  {
    ...
    USB_OUTEP(0)->DOEPTSIZ = (USB_OTG_DOEPTSIZ_PKTCNT & (1 << 19)) |
                             USB_OTG_DOEPTSIZ_STUPCNT | // allow max (3) setup pkt
                             (3 * 8); // transfer size
    USB_INEP (0)->DIEPCTL = USB_OTG_DIEPCTL_USBAEP | // active endpoint (always 1)
