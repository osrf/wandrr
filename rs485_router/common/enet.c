...
// hard-coded ip for now... 10.66.176.89
#ifndef ETH_IP_ADDR
  #define ETH_IP_ADDR 0x0a42b059
#endif
...
uint32_t g_host_ip_addr = 0x0a42b024; // TODO: overwrite this intelligently
...
static bool eth_dispatch_udp(const uint8_t *data, const uint16_t len)
{
  ...
  */
  if (port == 11333 && payload_len > 0)
  {
    ...
    else if (cmd == 3 && payload_len == 25) // wandrr robot 12-dmxl goals
    {
      // this will take a while to forward, so let's parse the xsens between
      #define INTER_PKT_GAP_US 20
      for (int i = 0; i < 5; i++) // left leg
      {
        dmxl_set_regs(0, i + 1, 30, 2, &payload[i*2+1]);
        delay_us(INTER_PKT_GAP_US);
        xsens_parse_rx_ring();
      }
      for (int i = 5; i < 10; i++) // right leg
      {
        dmxl_set_regs(1, i - 5 + 1, 30, 2, &payload[i*2+1]);
        delay_us(INTER_PKT_GAP_US);
        xsens_parse_rx_ring();
      }
      for (int i = 10; i < 12; i++) // neck
      {
        dmxl_set_regs(2, i - 10 + 1, 30, 2, &payload[i*2+1]);
        delay_us(INTER_PKT_GAP_US);
        xsens_parse_rx_ring();
      }
    }
