...
void stream(libusb_device_handle *h)
{
  ...
  while (!g_done)
  {
    ...
    if (nrx == 64 && rx_msg[0] == 0)
    {
      ...
      halls = rx_msg[36];
      memcpy(&therm_celsius, &rx_msg[40], 4);
      //uint8_t a = rx_msg[16];
      //printf("rx t = %12d  ang = %04x  ang2 = %04x\n", 
      //       t, (unsigned)angle, (unsigned)angle2);
      //printf("angle = %.3f v = %.3f ns = %d\n", angle, vel, num_samp);
      //printf("%d %d %d\n",
      //       hall_array[0], hall_array[1], hall_array[2]);
      static bool init_complete = false;
      static int count = 0;
      if (init_complete && count++ % 100 == 0)
      {
        printf("%10.3f %10.3f %10.3f %10.3f   %10.3f  %10.3f  %10.3f  %8d %d\n", 
               therm_celsius, vel[0], vel[1], vel[2], 
               angle[0], angle[1], angle[2], raw_angle, halls);
        /*
