...
public:
  ...
  virtual void tick(MiniState *ms)
  {
    const double CYCLE_TIME = 3.0;
    double frac_t = fmod(ros::Time::now().toSec(), CYCLE_TIME) / 
                    CYCLE_TIME * 2.0 * 3.14159;
    goals[L_SHOULDER_Y] = 0.2 * sin(frac_t);
    goals[R_SHOULDER_Y] = 0.2 * sin(frac_t);
  }
};

class Hug : public ArmController
{
public:
  Hug() : ArmController("hug", "plz hug me, i have no friends") 
  { 
    goals[L_SHOULDER_Y] =  0   ;
    goals[L_SHOULDER_X] =  1.3 ;
    goals[L_SHOULDER_Z] =  0.2 ;
    goals[L_ELBOW_Y   ] =  0   ;
    goals[R_SHOULDER_Y] =  0   ;
...
public:
  ...
  virtual void tick(MiniState *ms)
  {
    const double CYCLE_TIME = 1.0;
    double frac_t = fmod(ros::Time::now().toSec(), CYCLE_TIME) / 
                    CYCLE_TIME * 2.0 * 3.14159;
    goals[L_ELBOW_Y] =  1.57 + 0.1 * sin(frac_t);
    goals[R_ELBOW_Y] = -1.57 + 0.1 * sin(frac_t);
  }
};
...
int main(int argc, char **argv)
{
  ...
  arm_controllers.push_back(new Hug());
